...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Disease');
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    summary: 'Current global COVID-19 case and death totals, from the disease.sh API.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  },
  {
    ...
    summary: 'Current COVID-19 case and death totals for one country, from the disease.sh API.',
    inputSchema: {
      type: 'object',
      properties: {
        country: {
  ...
  },
  {
    ...
    summary: 'Daily COVID-19 case history for a country or the world, from the disease.sh API.',
    inputSchema: {
      type: 'object',
      properties: {
        country: {
  ...
  },
  {
    ...
    summary: 'COVID-19 vaccine doses administered for a country, from the disease.sh API.',
    inputSchema: {
      type: 'object',
      properties: {
        country: {
...
async function getGlobalStats() {
  ...
  if (!res.ok) throw await httpError(res, 'disease.sh error');

  const data = (await parseJson<unknown>(res, 'disease.sh')) as RawGlobalStats;
  return {
    cases: data.cases,
    deaths: data.deaths,
...
async function getCountryStats(country: string) {
  const res = await pwFetch(`${BASE_URL}/countries/${encodeURIComponent(country)}`);
  if (!res.ok) throw await httpError(res, 'disease.sh error');

  const data = (await parseJson<unknown>(res, 'disease.sh')) as RawCountryStats;
  return {
...
async function getHistorical(country: string, days: number) {
  ...
  // disease.sh 404s a country it can't place OR one that simply has no timeline,
  // and it says both in one sentence. Either way it is an answer about the
  // argument, not a failure of ours — but it was landing in blob4='error', the
  // tier reserved for Pipeworx defects, so a caller typing a country we don't
  // cover showed up on the problem-tools list as a broken tool.
  if (res.status === 404) {
    throw new Error(
      `user_error: disease.sh has no COVID timeline for "${country}". It matches country names, ISO2/ISO3 codes ` +
        `and common aliases ("USA", "US", "United States" all work), so check the spelling — and note that some ` +
        `small territories are covered by get_country_stats but have no historical series at all. ` +
        `Omit \`country\` for the global timeline.`,
    );
  }
  if (!res.ok) throw await httpError(res, 'disease.sh error');

  const data = (await parseJson<unknown>(res, 'disease.sh')) as RawHistoricalAll | RawHistoricalCountry;
...
async function getVaccineStats(country?: string) {
  ...
  const data = (await parseJson<unknown>(res, 'disease.sh')) as RawVaccineCoverage | RawVaccineCountry;
